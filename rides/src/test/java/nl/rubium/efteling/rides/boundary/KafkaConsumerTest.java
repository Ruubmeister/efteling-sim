...
public class KafkaConsumerTest {
    ...
    public void eventsTopicListener_givenVisitorStepInRideLine_expectVisitorHandled()
        ...
        await().pollInterval(Duration.ofSeconds(1))
                ...
                .untilAsserted(
                        () ->
                                verify(rideControl, times(1))
                                        .handleVisitorSteppingInRideLine(any(), any()));
    }

    @Test
    public void eventsTopicListener_givenEmployeeChangedWorkplace_expectChangeHandled()
            throws Exception {
        // Using injected ObjectMapper bean
        var workplace =
                org.openapitools.client.model.WorkplaceDto.builder()
                        ...
                        .location(
                                org.openapitools.client.model.GridLocationDto.builder()
                                        .x(java.math.BigDecimal.valueOf(10))
                                        .y(java.math.BigDecimal.valueOf(20))
                                        .build())
                        .build();
        var payload =
                Map.of(
        ...
        await().pollInterval(Duration.ofSeconds(1))
                ...
                .untilAsserted(
                        () ->
                                verify(rideControl, times(1))
                                        .handleEmployeeChangedWorkplace(any(), any(), any()));
    }

    @Test
    public void eventsTopicListener_givenParkIsOpen_expectRidesAreOpened() throws Exception {
        var payload = Map.of("status", "OPEN");
        kafkaProducer.sendEvent(EventSource.PARK, EventType.STATUSCHANGED, payload);

        await().pollInterval(Duration.ofSeconds(1))
                .atMost(10, TimeUnit.SECONDS)
                .untilAsserted(() -> verify(rideControl, times(1)).openRides());
    }

    @Test
    public void eventsTopicListener_givenParkIsClosed_expectRidesAreClosed() throws Exception {
        var payload = Map.of("status", "CLOSED");
        kafkaProducer.sendEvent(EventSource.PARK, EventType.STATUSCHANGED, payload);

        await().pollInterval(Duration.ofSeconds(1))
                .atMost(10, TimeUnit.SECONDS)
                .untilAsserted(() -> verify(rideControl, times(1)).closeRides());
    }
}
